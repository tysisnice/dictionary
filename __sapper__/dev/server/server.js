...
var toggleFavorite = function (_id) {
    ...
    savedById.update(function (state) {
        if (state.includes(_id)) {
            return state.filter(function (e) { return e !== _id; });
        }
        state.push(_id);
...
const WordCard = create_ssr_component(($$result, $$props, $$bindings, $$slots) => {
  ...
  const { savedById, toggleFavorite } = favorites; validate_store(savedById, 'savedById'); $savedById = get_store_value(savedById);
  ...
  let { entry, open } = $$props;
	...
	validate_store(savedById, 'savedById'); $savedById = get_store_value(savedById);

	return `<div class="container svelte-1k4yeau"${open ? " open" : "" }>
	  ${ open ? `<div class="corner-button open svelte-1k4yeau"${add_attribute("saved", $savedById.includes(entry._id), 0)}>★</div>
	    <p class="word svelte-1k4yeau">${escape(entry.word)}</p>
	    <p class="clarifyer svelte-1k4yeau">${escape(entry.clarifyer)}</p>
	    <div class="other-entries svelte-1k4yeau">
	      ${each(entry.otherWords, ({_id, word, clarifyer}, index) => `<hr class="svelte-1k4yeau">
	        <p class="word other svelte-1k4yeau">${escape(word)}</p>
	        <p class="clarifyer other svelte-1k4yeau">${escape(clarifyer)}</p>`)}
	    </div>` : `<div class="corner-button svelte-1k4yeau">+</div>
	    <p class="word svelte-1k4yeau">${escape(entry.word)}</p>
	    <p class="clarifyer svelte-1k4yeau">${escape(shrinkClarifyerText(entry.clarifyer))}</p>` }
	</div>`;
});
...
const css$2 = {
	code: "h2.svelte-1ig13xx{margin:30px 0px 15px 0px;max-width:95vw;width:360px;font-size:30px;font-weight:bold;color:#555}h2.top.svelte-1ig13xx{margin-top:20px}",
	map: "{\"version\":3,\"file\":\"index.svelte\",\"sources\":[\"index.svelte\"],\"sourcesContent\":[\"<script>\\n  import { flip } from 'svelte/animate';\\n  import WordCard from '../../components/WordCard.svelte';\\n  import { favorites } from '../../store';\\n  const { savedWords, wordOfTheDay } = favorites;\\n  $: faves = $savedWords.reverse();\\n</script>\\n\\n<h2 class=\\\"top\\\">Word of the Day</h2>\\n<WordCard entry={$wordOfTheDay} open=\\\"true\\\" />\\n<h2>My Saved Words</h2>\\n{#each faves as entry, i (entry._id)}\\n  <div animate:flip={{duration: 220}}>\\n    <WordCard {entry} open=\\\"true\\\" />\\n  </div>\\n{/each}\\n\\n\\n<style>\\n  h2 {\\n    margin: 30px 0px 15px 0px;\\n    max-width: 95vw;\\n    width: 360px;\\n    font-size: 30px;\\n    font-weight: bold;\\n    color: #555;\\n  }\\n  h2.top {\\n    margin-top: 20px;\\n  }\\n</style>\"],\"names\":[],\"mappings\":\"AAmBE,EAAE,eAAC,CAAC,AACF,MAAM,CAAE,IAAI,CAAC,GAAG,CAAC,IAAI,CAAC,GAAG,CACzB,SAAS,CAAE,IAAI,CACf,KAAK,CAAE,KAAK,CACZ,SAAS,CAAE,IAAI,CACf,WAAW,CAAE,IAAI,CACjB,KAAK,CAAE,IAAI,AACb,CAAC,AACD,EAAE,IAAI,eAAC,CAAC,AACN,UAAU,CAAE,IAAI,AAClB,CAAC\"}"
};
...
const Index$1 = create_ssr_component(($$result, $$props, $$bindings, $$slots) => {
  ...
  const { savedWords, wordOfTheDay } = favorites; validate_store(savedWords, 'savedWords'); $savedWords = get_store_value(savedWords); validate_store(wordOfTheDay, 'wordOfTheDay'); $wordOfTheDay = get_store_value(wordOfTheDay);

	$$result.css.add(css$2);

	validate_store(savedWords, 'savedWords'); $savedWords = get_store_value(savedWords);
	validate_store(wordOfTheDay, 'wordOfTheDay'); $wordOfTheDay = get_store_value(wordOfTheDay);

	let faves = $savedWords.reverse();

	return `<h2 class="top svelte-1ig13xx">Word of the Day</h2>
	${validate_component(WordCard, 'WordCard').$$render($$result, { entry: $wordOfTheDay, open: "true" }, {}, {})}
	<h2 class="svelte-1ig13xx">My Saved Words</h2>
...
const css$3 = {
	code: "div.svelte-zvwzgj{margin-top:-2px;-webkit-transform:rotate(-45deg);-moz-transform:rotate(-45deg);-o-transform:rotate(-45deg);transform:rotate(-45deg);font-size:34px;font-weight:bold;user-select:none}",
	map: "{\"version\":3,\"file\":\"SearchIcon.svelte\",\"sources\":[\"SearchIcon.svelte\"],\"sourcesContent\":[\"\\n<style>\\n  div {\\n    margin-top: -2px; \\n    -webkit-transform: rotate(-45deg); \\n    -moz-transform: rotate(-45deg); -o-transform: rotate(-45deg); \\n    transform: rotate(-45deg); \\n    font-size: 34px; \\n    font-weight: bold;\\n    user-select: none;\\n  }\\n</style>\\n\\n<div class=\\\"search-icon\\\">&#9906;</div>\"],\"names\":[],\"mappings\":\"AAEE,GAAG,cAAC,CAAC,AACH,UAAU,CAAE,IAAI,CAChB,iBAAiB,CAAE,OAAO,MAAM,CAAC,CACjC,cAAc,CAAE,OAAO,MAAM,CAAC,CAAE,YAAY,CAAE,OAAO,MAAM,CAAC,CAC5D,SAAS,CAAE,OAAO,MAAM,CAAC,CACzB,SAAS,CAAE,IAAI,CACf,WAAW,CAAE,IAAI,CACjB,WAAW,CAAE,IAAI,AACnB,CAAC\"}"
};

const SearchIcon = create_ssr_component(($$result, $$props, $$bindings, $$slots) => {
	$$result.css.add(css$3);

	return `<div class="search-icon svelte-zvwzgj">⚲</div>`;
});

/* src/routes/search/index.svelte generated by Svelte v3.9.0 */

const css$4 = {
	code: ".search-bar.svelte-1idlthm{display:flex;justify-content:space-between;width:435px;height:44px;margin:-5px 0 15px 0;border:2px solid #ddd;border-radius:40px;overflow:hidden}.search-bar.svelte-1idlthm>input.svelte-1idlthm{width:100%;border:none;padding:5px;outline:none;font-size:24px;padding-left:18px}.search-bar.svelte-1idlthm>div.svelte-1idlthm{font-size:40px;padding:10px;margin:-14px 6px auto auto;cursor:pointer;background:pink}@media(max-width: 550px){.search-bar.svelte-1idlthm{width:calc(100vw - 40px)}}",
...
const Index$2 = create_ssr_component(($$result, $$props, $$bindings, $$slots) => {
  ...
  const { searchQuery, resultsBySearch, updateSearchQuery } = lookup$1; validate_store(searchQuery, 'searchQuery'); $searchQuery = get_store_value(searchQuery); validate_store(resultsBySearch, 'resultsBySearch'); $resultsBySearch = get_store_value(resultsBySearch);

	$$result.css.add(css$4);

	validate_store(searchQuery, 'searchQuery'); $searchQuery = get_store_value(searchQuery);
	validate_store(resultsBySearch, 'resultsBySearch'); $resultsBySearch = get_store_value(resultsBySearch);

	return `${($$result.head += `<title>Search</title>`, "")}



	<div class="search-bar svelte-1idlthm">
	  <input type="text"${add_attribute("value", $searchQuery, 0)} class="svelte-1idlthm">
	  <div class="svelte-1idlthm">${validate_component(SearchIcon, 'SearchIcon').$$render($$result, {}, {}, {})}</div>
	</div>
...
const Index$5 = create_ssr_component(($$result, $$props, $$bindings, $$slots) => {
  ...
  const { updateSelectedLetter, selectedLetter, resultsByLetter } = lookup$1; validate_store(selectedLetter, 'selectedLetter'); $selectedLetter = get_store_value(selectedLetter); validate_store(resultsByLetter, 'resultsByLetter'); $resultsByLetter = get_store_value(resultsByLetter);
  const { alphabet, language } = dictionary; validate_store(alphabet, 'alphabet'); $alphabet = get_store_value(alphabet); validate_store(language, 'language'); $language = get_store_value(language);

	$$result.css.add(css$7);

	validate_store(alphabet, 'alphabet'); $alphabet = get_store_value(alphabet);
	validate_store(language, 'language'); $language = get_store_value(language);
	validate_store(selectedLetter, 'selectedLetter'); $selectedLetter = get_store_value(selectedLetter);
	validate_store(resultsByLetter, 'resultsByLetter'); $resultsByLetter = get_store_value(resultsByLetter);

	let abc = $alphabet[$language.selected];
	let aToZ = `${abc[0].toUpperCase()}-${abc[abc.length - 1].toUpperCase()}`;
	let abcList = [aToZ, ...abc.split('')];

	return `${ $selectedLetter && $selectedLetter !== aToZ ? `<div class="az-container svelte-1ozkh1t">
	    ${each(abcList, (letter, i) => `<div class="az-item svelte-1ozkh1t"${letter === $selectedLetter ? " selected" : "" }>
	        ${escape(letter)}
	      </div>`)}
	  </div>
	  <h2 class="svelte-1ozkh1t">${escape($selectedLetter.toUpperCase())}</h2>
	  ${ $resultsByLetter.length === 0 ? `<h3 class="svelte-1ozkh1t">No results for "${escape($selectedLetter.toUpperCase())}"</h3>` : `` }
	  ${each($resultsByLetter, ({entry, open}, i) => `${validate_component(WordCard, 'WordCard').$$render($$result, { open: open, entry: entry }, {}, {})}`)}` : `<div class="az-container-open svelte-1ozkh1t">
	    ${each(abcList.slice(1), (letter, i) => `<div class="az-item open svelte-1ozkh1t"${letter === $selectedLetter ? " selected" : "" }>
	        ${escape(letter)}
	      </div>`)}
	  </div>` }`;
});
...
const css$8 = {
	code: ".tabbar.svelte-bxhcwq{--height:36px;display:flex;justify-content:space-around;position:fixed;bottom:0;right:0;left:0;height:var(--height)}a.svelte-bxhcwq{text-decoration:none;color:black;width:25%;text-align:center;height:inherit;background:pink;line-height:var(--height)}a.selected.svelte-bxhcwq{color:dodgerblue}@media(min-width: 600px){.tabbar.svelte-bxhcwq{--height:inherit;position:relative;width:150px}a.svelte-bxhcwq{width:50px}}",
	map: "{\"version\":3,\"file\":\"Tabbar.svelte\",\"sources\":[\"Tabbar.svelte\"],\"sourcesContent\":[\"\\n<script>\\n  export let segment;\\n</script>\\n\\n\\n<div class=\\\"tabbar\\\">\\n  <a class='{segment === \\\"abc\\\" ? \\\"selected\\\" : \\\"\\\"}' href='abc'>\\n    A-Z\\n  </a>\\n  <a class='{segment === \\\"search\\\" ? \\\"selected\\\" : \\\"\\\"}' href='search'>\\n    Search\\n  </a>\\n  <a class='{segment === \\\"favorites\\\" ? \\\"selected\\\" : \\\"\\\"}' href='favorites'>\\n    Star\\n  </a>\\n</div>\\n\\n\\n<style>\\n\\t.tabbar {\\n    --height: 36px;\\n\\t\\tdisplay: flex;\\n\\t\\tjustify-content: space-around;\\n\\t\\tposition: fixed;\\n\\t\\tbottom: 0; right: 0; left: 0;\\n    height: var(--height);\\n\\t}\\n\\n\\ta {\\n\\t\\ttext-decoration: none;\\n\\t\\tcolor: black;\\n    width: 25%;\\n    text-align: center;\\n    height: inherit;\\n    background: pink;\\n    line-height: var(--height);\\n\\t}\\n\\n\\ta.selected {\\n\\t\\tcolor: dodgerblue;\\n\\t}\\n  \\n\\n  @media (min-width: 600px) {\\n    .tabbar {\\n      --height: inherit;\\n      position: relative;\\n      width: 150px;\\n    }\\n    a {\\n      width: 50px;\\n    }\\n  }\\n</style>\"],\"names\":[],\"mappings\":\"AAoBC,OAAO,cAAC,CAAC,AACN,QAAQ,CAAE,IAAI,CAChB,OAAO,CAAE,IAAI,CACb,eAAe,CAAE,YAAY,CAC7B,QAAQ,CAAE,KAAK,CACf,MAAM,CAAE,CAAC,CAAE,KAAK,CAAE,CAAC,CAAE,IAAI,CAAE,CAAC,CAC1B,MAAM,CAAE,IAAI,QAAQ,CAAC,AACxB,CAAC,AAED,CAAC,cAAC,CAAC,AACF,eAAe,CAAE,IAAI,CACrB,KAAK,CAAE,KAAK,CACV,KAAK,CAAE,GAAG,CACV,UAAU,CAAE,MAAM,CAClB,MAAM,CAAE,OAAO,CACf,UAAU,CAAE,IAAI,CAChB,WAAW,CAAE,IAAI,QAAQ,CAAC,AAC7B,CAAC,AAED,CAAC,SAAS,cAAC,CAAC,AACX,KAAK,CAAE,UAAU,AAClB,CAAC,AAGA,MAAM,AAAC,YAAY,KAAK,CAAC,AAAC,CAAC,AACzB,OAAO,cAAC,CAAC,AACP,QAAQ,CAAE,OAAO,CACjB,QAAQ,CAAE,QAAQ,CAClB,KAAK,CAAE,KAAK,AACd,CAAC,AACD,CAAC,cAAC,CAAC,AACD,KAAK,CAAE,IAAI,AACb,CAAC,AACH,CAAC\"}"
};
...
const Tabbar = create_ssr_component(($$result, $$props, $$bindings, $$slots) => {
	...
	return `<div class="tabbar svelte-bxhcwq">
	  <a class="${escape(null_to_empty(segment === "abc" ? "selected" : ""))} svelte-bxhcwq" href="abc">
	    A-Z
	  </a>
	  <a class="${escape(null_to_empty(segment === "search" ? "selected" : ""))} svelte-bxhcwq" href="search">
	    Search
	  </a>
	  <a class="${escape(null_to_empty(segment === "favorites" ? "selected" : ""))} svelte-bxhcwq" href="favorites">
	    Star
	  </a>
	</div>`;
});
...
const css$9 = {
	code: "nav.svelte-mu3wji{--nav-heigth:42px;position:fixed;top:0;left:0;right:0;height:var(--nav-heigth);background:dodgerblue;color:white;z-index:10;padding-right:0;display:flex;justify-content:space-between}h1.svelte-mu3wji{margin-left:6px;line-height:calc(var(--nav-heigth) - 2px);font-size:26px}div.svelte-mu3wji{display:flex;align-items:center;height:inherit}button.svelte-mu3wji{cursor:pointer;font-size:16px;height:calc(var(--nav-heigth) - 10px);margin:5px;border:none;border-radius:6px;box-shadow:0px 2px 5px 0px rgba(50,50,50,0.2);transition:0.05s}button.primary.svelte-mu3wji{width:40px;margin:5px 9px 5px 9px}button.secondary.svelte-mu3wji{width:48px}",
...
const Nav = create_ssr_component(($$result, $$props, $$bindings, $$slots) => {
	...
	return `<nav class="svelte-mu3wji">
		<h1 class="svelte-mu3wji">Joes house</h1>
		<div class="svelte-mu3wji">
			${validate_component(Tabbar, 'Tabbar').$$render($$result, { segment: segment }, {}, {})}
			<button class="${escape(null_to_empty(langButtonClass))} svelte-mu3wji">
				${escape(langText)}
			</button>
		</div>
	</nav>`;
...
const css$a = {
	code: "main.svelte-1s3yue9{position:relative;width:100%;background-color:white;padding:2em;margin:0 auto;box-sizing:border-box;margin-top:40px;display:flex;flex-direction:column;justify-content:center;align-items:center}",
